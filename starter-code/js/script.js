...
document.getElementById("toggle").addEventListener("click", () => {
	//Have to target the input not label
				...
				.then((data) => {
					if (data.error) {
						...
						let wordText = document.querySelector("#word-text"),
							wordPronounce =
								document.querySelector("#word-pronounce"),
						...
						for (let p = 0; p < data[0].phonetics.length; p++) {
							if (
								data[0].phonetics[p].text &&
								data[0].phonetics[p].text !== "" &&
								data[0].phonetics[p].audio &&
								data[0].phonetics[p].audio !== ""
							) {
								wordPronounce.textContent =
									data[0].phonetics[p].text;
						...
						for (let m = 0; m < data[0].meanings.length; m++) {
							...
							for (let j = 0; j < mergedMeanings.length; j++) {
								...
								if (
									currentDict.partOfSpeech ===
									mergedDict.partOfSpeech
								) {
									mergedDict.definitions =
										mergedDict.definitions.concat(
											currentDict.definitions
										);
									mergedDict.synonyms =
										mergedDict.synonyms.concat(
											currentDict.synonyms
										);
									mergedDict.antonyms =
										mergedDict.antonyms.concat(
											currentDict.antonyms
										);
									merged = true;
									break;
								}
						...
						for (let m = 0; m < mergedMeanings.length; m++) {
							...
							for (
								let d = 0;
								d < mergedMeanings[m].definitions.length;
								d++
							) {
								const meaningLi = document.createElement("li");
								const meaningLiP = document.createElement("p");

								meaningLiP.textContent =
									mergedMeanings[m].definitions[d].definition;
							...
							if (
								mergedMeanings[m].synonyms &&
								mergedMeanings[m].synonyms.length > 0
							) {
								const synonymDiv =
								...
								synonymWords.textContent =
									mergedMeanings[m].synonyms.join(", "); //to convert the array of synonyms into a comma separated string
							...
							if (
								mergedMeanings[m].antonyms &&
								mergedMeanings[m].antonyms.length > 0
							) {
								const antonymDiv =
									document.createElement("div");
								...
								antonymWords.textContent =
									mergedMeanings[m].antonyms.join(", "); //to convert the array of antnonyms into a comma separated string
						...
						imgNewWindow.src =
							"./starter-code/assets/images/icon-new-window.svg";
